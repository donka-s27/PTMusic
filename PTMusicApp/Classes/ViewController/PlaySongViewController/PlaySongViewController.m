...
-(void)viewWillAppear:(BOOL)animated{
    adBanner.view.backgroundColor = [UIColor redColor];
...
-(void) fixMultiScreenSize{
    ...
    self.pageView.frame=CGRectMake(self.pageView.frame.origin.x, self.pageView.frame.origin.y, SCREEN_WIDTH_PORTRAIT, screen_height-self.pageView.frame.origin.y-footerHeightView-GAD_SIZE_320x50.height + 30);
    
    
    self.footerView.frame = CGRectMake(0,screen_height-footerHeightView-GAD_SIZE_320x50.height, SCREEN_WIDTH_PORTRAIT, footerHeightView); // the player always is bottom
...
-(void)play{
    ...
    if ([self.song.songId isEqualToString: currentAudio]) {
        ...
        if([Util objectForKey:[NSString stringWithFormat:@"%@_localFile",self.song.songId]])
        {
            self.song.localMP3 = [Util objectForKey:[NSString stringWithFormat:@"%@_localFile",self.song.songId]];
...
- (IBAction)onDownload:(id)sender {
    ...
    if (self.song.localMP3.length > 0) {
...
- (IBAction)onRightAction:(id)sender {
    ...
    if(dropDown == nil) {
        ...
        dropDown = [[NIDropDown alloc]showDropDown:button1 :&f :[NSMutableArray arrayWithArray:@[@"Make available offline",@"Add PlayList"]] :nil :@"down"];
